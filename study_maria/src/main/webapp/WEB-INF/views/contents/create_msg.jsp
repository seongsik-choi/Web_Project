    ...
    <UL>
      <c:choose>
        <c:when test="${param.cnt == 1}">
          <LI class='li_none'>
            <span class="span_success">스터지 지원서를 등록했습니다.</span>
          </LI>
        </c:when>
        <c:otherwise>
          <LI class='li_none_left'>
            <span class="span_fail">스터지 지원서 등록에 실패했습니다.</span>
          </LI>
          <LI class='li_none_left'>
      ...
      <LI class='li_none'>
        ...
        <c:choose>
          <c:when test="${param.cnt == 1 }">
            <button type='button' 
                         onclick="location.href='./create.do?stuno=${param.stuno}'"
                         class="btn btn-primary">새로운 스터지 지원서 등록</button>
          </c:when>
          <c:otherwise>
