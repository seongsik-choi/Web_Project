...
<script type="text/javascript">
  ...
  $(function() { // 자동 실행
    ...
    $('#btn_checkEMAIL').on('click', checkEMAIL); // 이메일 체크  
    $('#btn_checkVNAME').on('click', checkVNAME); // 별명 체크  
  ...
  // jQuery ajax 요청(EMAIL 중복 확인 요청)
  function checkEMAIL() {
    // $('#btn_close').attr("data-focus", "이동할 태그 지정");
    
    var frm = $('#frm'); // id가 frm인 태그 검색
    var email = $('#email', frm).val(); // frm 폼에서 id가 'id'인 태그 검색
    var params = '';
    var msg = '';

    if ($.trim(email).length == 0) { // email를 입력받지 않은 경우
      msg = '· email를 입력하세요.<br> · ex)OOOOO@naver.com';
      $('#btn_close').attr("data-focus", "email"); // data-focus에 id 적용 : email에서 아직 안넘어갔기에!
    ...
    } else {  // when email is entered(정상인 경우)
      params = 'email=' + email;
      // var params = $('#frm').serialize(); // 직렬화, 폼의 데이터를 키와 값의 구조로 조합
      // alert('params: ' + params);

      $.ajax({
        url: './checkEMAIL.do', // spring controller url
        type: 'get',  // post
        cache: false, // 응답 결과 임시 저장 취소
        async: true,  // true: 비동기 통신
        dataType: 'json', // 응답 형식: json, html, xml...
        ...
        success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
          ...
          // 새로운 moodal, cnt>0 -> email 중복시 모달
          if (rdata.cnt > 0) { 
            $('#modal_content').attr('class', 'alert alert-danger'); // Bootstrap CSS 변경
            msg = "이미 사용중인 email 입니다."; 
            $('#btn_close').attr("data-focus", "email"); // data-focus에 id 적용
          } else {
            $('#modal_content').attr('class', 'alert alert-success'); // Bootstrap CSS 변경
            msg = "사용 가능한 email 입니다.";
            $('#btn_close').attr("data-focus", "vname");
            // $.cookie('checkId', 'TRUE'); // Cookie 기록
          }
  ...
  // jQuery ajax 요청(vname 중복 확인 요청)
  function checkVNAME() {
    // $('#btn_close').attr("data-focus", "이동할 태그 지정");
    
    var frm = $('#frm'); // id가 frm인 태그 검색
    var vname = $('#vname', frm).val(); // frm 폼에서 vname이 'vname'인 태그 검색
    var params = '';
    var msg = '';

    if ($.trim(vname).length == 0) { // email를 입력받지 않은 경우
      msg = '· 별명을 입력하세요.<br> · 별명은 자유입니다.';
      $('#btn_close').attr("data-focus", "vname"); // data-focus에 id 적용 : email에서 아직 안넘어갔기에!
      ...
      $.ajax({
        url: './checkVNAME.do', // spring controller url
        type: 'get',  // post
        cache: false, // 응답 결과 임시 저장 취소
        async: true,  // true: 비동기 통신
        dataType: 'json', // 응답 형식: json, html, xml...
        ...
        success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
          ...
          if (rdata.cnt > 0) { 
            $('#modal_content').attr('class', 'alert alert-danger'); // Bootstrap CSS 변경
            msg = "이미 사용중인 별명 입니다."; 
            $('#btn_close').attr("data-focus", "vname"); // data-focus에 vname 적용
          } else {
            $('#modal_content').attr('class', 'alert alert-success'); // Bootstrap CSS 변경
            msg = "사용 가능한 별명 입니다.";
            $('#btn_close').attr("data-focus", "job");
            // $.cookie('checkId', 'TRUE'); // Cookie 기록
          }
...
<jsp:include page="../menu/top.jsp" flush='false' />
  ...
  <FORM name='frm' id='frm' method='POST' action='./create.do' class="form-horizontal"
               enctype="multipart/form-data"> <!-- 파일 추가를 위한 문 -->
    ...
    <!-- 새로 추가한 email, vname, job, image -->
    <div class="form-group">
      <label for="address2" class="col-md-2 control-label" style='font-size: 0.9em;'>이메일*</label>    
      <div class="col-md-10">
        <input type='text' class="form-control" name='email' id='email' 
                   value='' style='width: 60%;' placeholder="ex)ooooo@naver.com">
        <button type='button' id="btn_checkEMAIL" class="btn btn-info btn-md">이메일 중복확인</button>
        <SPAN id='id_span'></SPAN> <!-- ID 중복 관련 메시지 -->                           
      </div>
    </div>   

    <div class="form-group">
      <label for="address2" class="col-md-2 control-label" style='font-size: 0.9em;'>별명</label>    
      <div class="col-md-10">
        <input type='text' class="form-control" name='vname' id='vname' 
                   value='' style='width: 50%;' placeholder="별명">
        <button type='button' id="btn_checkVNAME" class="btn btn-info btn-md">별명 중복확인</button>
        <SPAN id='id_span'></SPAN> <!-- ID 중복 관련 메시지 -->                         
      </div>
    </div>   
    
    <div class="form-group">
      <label for="address2" class="col-md-2 control-label" style='font-size: 0.9em;'>직업</label>    
      <div class="col-md-10">
        <input type='text' class="form-control" name='job' id='job' 
                   value='' style='width: 50%;' placeholder="직업">                   
      </div>
    </div>              
